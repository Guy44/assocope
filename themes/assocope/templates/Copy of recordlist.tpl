    ...
    <td align="left">
        <table id="{$listid}" class="recordlist" cellspacing=0>
            ...
            {foreach from=$rows item=row}
              <tr id="{$row.id}" class="row{if $row.rownum % 2 == 0 }1{else}2{/if}" {if $row.background!=""}style="-webkit-print-color-adjust:exact; background-color:{$row.background}" {/if}
               {if $row.pagebreak!=""}style="{$row.pagebreak}" {/if}
