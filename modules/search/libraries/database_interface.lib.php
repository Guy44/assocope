 ...
 */
require_once atkconfig("atkroot") . 'modules/search/libraries/dbi/' . $GLOBALS['cfg']['Server']['extension'] . '.dbi.lib.php';
    ...
    if (! empty($server_language) && isset($encodings[$server_language])) {
        if (function_exists('iconv')) {
            if ((@stristr(PHP_OS, 'AIX')) && (@strcasecmp(ICONV_IMPL, 'unknown') == 0) && (@strcasecmp(ICONV_VERSION, 'unknown') == 0)) {
                require_once atkconfig("atkroot") . 'modules/search/libraries/iconv_wrapper.lib.php';
