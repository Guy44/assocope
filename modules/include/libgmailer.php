 ...
 * Other constants.
*/
define("GM_VER", "1.0 Beta 10apr07");
...
/* 		exit; */
		...
		if (strpos($cookies, "HID=") !== false or ($this->domain and strpos($this->gmail_data,"AuthEventSource=Internal&auth=") !== false)) {
			...
			///Debugger::say("hosted phase location match: ".print_r($matches,true));
...
/* 						strpos($second, "You cannot log into Gmail using your Google Account username and password.") !== false */
		...
		if ($this->domain) {
			...
			if ($redir_url != "") {
				///Debugger::say("host domain cookies: "."GoogleAccountsLocale_session=en; ".$cookies);
				...
				if (preg_match("/Location:\s*((http(s)?:\/\/)?(.*?))\n/",$second,$matches)) {
					...
					///Debugger::say("second (open_basedir post 2) phase cookies: ".print_r($data,true));
					...
					///Debugger::say("second (open_basedir 2) phase: ".print_r($second,true));
					///Debugger::say("second (open_basedir 2) phase cookies: ".print_r($data,true));
				}
			}
