...
class gChart{
	...
	public function setProperty($key, $value, $append = false, $dataSetSeparator = '|') {
		if ($append && isset($this->chart[$key])) {
			$this -> chart[$key] = $this -> chart[$key].$dataSetSeparator.utf8_encode($value);
		} else {
			$this -> chart[$key] = utf8_encode($value);
		}
	}
	/**
...
class gLineChart extends gChart{
	...
	/**
	 * @brief Sets �péseur des traits
	 *
	 * @param $labels Array
	 */
	public function setLineStyles($labels) {
		//	$this -> setProperty('chdl', urlencode($this->encodeData($this->getApplicableLabels($labels),"|")));
		$this -> setProperty('chls', $this->encodeData($this->getApplicableLabels($labels),"|"));
	}
}
