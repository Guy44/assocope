...
/*    function putCache($address, $lon, $lat) {
     ...
     */
    // GG
	function putCache($idlieu, $lon, $lat, $type) {
		$db = & atkGetDb();
		if (!isset ($lon) or !isset ($lat)) {
			return false;
		} else {
			if ($type == 'ville') {
				$db->query("Update type_cp_ville_pays set longitude=$lon, latitude=$lat, date_modification='".gmdate(c)."' where id =$idlieu ");
			} else {
				$db->query("Update lieu set longitude=$lon, latitude=$lat, date_modification='".gmdate(c)."' where id =$idlieu ");
			}
			return true;
		}
