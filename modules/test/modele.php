...
//print_r($ubioHT);             
        ...
        switch( $_REQUEST['cmd'] ) {
                ...
                case 'taxagrid':
                        ...
                        foreach( $list as $str ) {
//                              print_r($str);
//                              print "http://ecat-dev.gbif.org/ws/n/" . urlencode($str['name']) . "<br>";
//                              $tmp = json_decode(file_get_contents("http://ecat-dev.gbif.org/ws/name/" . str_replace("+", "%20", urlencode($str['name'])) . "/" . $rkey), true);
                                        $tmp = json_decode(file_get_contents("http://names.gbif.org/ws/usage/?rkey=1&q=" . str_replace("+", "%20", urlencode($str['name'])) . "&pagesize=10&rank=s&showRanks=kpcofgs"), true);
//print_r($tmp);                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         
                                if (is_array($tmp['data'][0])) {
                                foreach($tmp['data'][0] as $field => $value) {
                                        if ($value == null) {
                                                $tmp['data'][0][$field] = '';
                                        }
                                }
